...
async def on_message(message):
    ...
    if message.author == client.user:
        ...
        embed.add_field(name="[gaw/pgaw/cvw/tnn] <6 fig lat> <6 fig lon>",
                        value="Looks up all units in a 5nm range around the coordinates and returns their position and type.")
        embed.add_field(name="[gaw/pgaw/cvw/tnn] <6 fig lat> <6 fig lon> <list of names>",
        ...
        embed.add_field(name="[gaw/pgaw/cvw/tnn] lookup <exact ingame name>",
                        value="Looks up the target user and returns the nearest group of enemies to them.")
        embed.add_field(name="list [gaw/pgaw/cvw/tnn]",
        ...
        if message.content.lower().startswith("rick"):
            ...
            elif message.content.lower().split(" ")[1] == "tnn":
                server_address = SERVER.TNN
            ...
            if check_valid(message):
                if check_if_assign(message) == "names":
                    ...
                    if any_targets is False:
                        await client.send_message(message.channel,
                                                  content="There were no targets detected within 5nm of that point.")
        elif message.content.lower().startswith("tnn"):
            if check_valid(message):
                if check_if_assign(message) == "names":
                    grouped = message.content.split(" ")
                    print("Detected Lat: " + str(grouped[1]))
                    ...
                    grouped = message.content.split(" ")
                    list_of_names = grouped[3:]
                    list_of_targets = collect_sorted_targets(grouped[1], grouped[2], SERVER.TNN)
                    ...
                    embed_one = build_embed(grouped[1], grouped[2], 1, SERVER.TNN)
                    embed_two = build_embed(grouped[1], grouped[2], 2, SERVER.TNN)
                    embed_three = build_embed(grouped[1], grouped[2], 3, SERVER.TNN)
                    embed_four = build_embed(grouped[1], grouped[2], 4, SERVER.TNN)
                    embed_five = build_embed(grouped[1], grouped[2], 5, SERVER.TNN)
                    embed_six = build_embed(grouped[1], grouped[2], 6, SERVER.TNN)
